...
class Reg_feature_selector():
    ...
    """
    Selects useful features. Several strategies are possible (filter and wrapper methods). Works for regression problems only.
    ...
    def set_params(self,**params):
        ...
        for k,v in params.items():
            if k not in self.get_params():
                warnings.warn("Invalid parameter a for feature selector Reg_feature_selector. Parameter IGNORED. Check the list of available parameters with `feature_selector.get_params().keys()`")
            else:
                setattr(self,k,v)
